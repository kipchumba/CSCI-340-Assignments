...
Hitter::Hitter(std::string s,int w,int h,char b,int atB,int ht) : BaseballPlayer(s,w,h) {
	bats=b;
	atBats=atB;
	hits=ht;
	batAvg=utility();
...
void Hitter::print_player(std::ofstream& outFile){
	...
	/** writing to outFile**/
	outFile<<name<<" "<<name<<" "<<height<<" "<<weight<<" "<<bats<<" "<<hits<<" "<<atBats<<"\n";
}
...
bool Hitter::goodHitter(){
	return (batAvg>=0.2795);
}
